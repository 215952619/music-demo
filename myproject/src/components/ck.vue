    ...
    <div class="player-controll">
      ...
      <div class="player-info" @click="showDetails">
        ...
        <div class="songsTitle">
          <p>
            <span v-text="songsInfo.name"></span>-
            <span v-text="songsInfo.ar[0].name"></span>
          </p>
...
<img :src="songsInfo.al.picUrl" alt="">
            ...
            <div class="showLrc">
              <ul v-html="Lrcs" class="lrc" v-bind:style="{top:LrcTop}"></ul>
            </div>
          </div>
      ...
      <div class="player-list-bottom" v-if="list">
        ...
        <ul class="play-list-ul">
          <li
            ...
            :key="item.id"
            :style="{color:item.id==activeId?'#fff':'#f4ea2a'}"
          >
            <i class="play-list-index">{{index+1}}</i>
            <p class="play-list-name" @click="setUrl(item.id)">
              <span v-text="item.name"></span>-
              <span
                class="play-list-singer"
                v-text="item.ar[0].name"
                :style="{color:item.id==activeId?'#fff':'#f4ea2a'}"
              ></span>
            </p>
            ...
            <li class="discuss-li" v-for="(item,index) in discuss.hotComments" :key="index">
              <div class="discuss-li-title">
                ...
                <div class="discuss-li-name">
                  ...
                  <p
                    class="discuss-li-p"
                    v-text="new Date(item.time).toLocaleDateString().substr(0,10).replace(/\//g,'-')"
                  ></p>
                </div>
              </div>
...
export default {
  ...
  computed: {
    ...mapGetters(['url', 'songsInfo', 'Lrc', 'Lrcs', 'playList', 'activeId'])
  },
  watch: {
    timeLine(n, o) {
      this.value = n;
  ...
  methods: {
    ...
    setUrl(id) {
      ...
      // 获取歌曲信息
      this.$http
        .get(this.config.api + '/song/detail', {
          params: {
            ids: id
          }
        })
    ...
    getDiscuss(id) {
      ...
      var _this = this;
      this.$http
        .get(this.config.api + '/comment/music', {
          params: {
            id: id
          }
        })
