    ...
    <div>
        ...
        <div class="player">
            ...
            <div class="info">
                ...
                <p><span>{{getMusicTime(current_time)}}</span>/<span>{{getMusicTime(song_time)}}</span></p>
                <p class="progress" @click="jumpTime()"><span class="bar"
                 :style="{ width : current_time / song_time * 100 + '%'}"></span></p>
            </div>
            <div class="mode" @mouseover="mouseover=true" @mouseout="mouseover=false">
                <button @click="changeMode(index)" v-for="(mode,index) in modes" :key="index"
                v-show="index===current_mode||mouseover"></button>
            </div>
            <div class="volume">
